...
bool insertUser(CDBConn *pDBConn, int id)
{
    bool bRet = false;
    string strSql;
    strSql = "insert into IMUser(`sex`,`name`,`domain`,`nick`,`password`,`salt`,`phone`,`email`,`company`,`address`,`avatar`,`sign_info`,`departId`,`status`,`created`,`updated`) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    if (stmt->Init(pDBConn->GetMysql(), strSql))
    {
        ...
        string strName = "廖庆富" + int2string(id_index);    // 真名
        string strDomain = "minghua" + int2string(id_index); // 花名拼音
        string strNick = "明华";                             // 花名
        string strOutPass = "987654321";                     // 密码
        string strSalt = "abcd";                             //
